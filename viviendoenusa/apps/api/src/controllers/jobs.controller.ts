...
// 🚀 Importamos las tablas con sus alias de seguridad
import { jobs, users, rating as ratingTable, reviews as reviewsTable } from "../../../../packages/db/src/schema"; 
import { eq, desc, sql } from "drizzle-orm"; 
// 🚀 Importamos 'alias' para poder hacer un doble JOIN a la misma tabla 'users'
...
// Definimos el alias para la tabla de usuarios que hacen la reseña
const reviewerUsers = alias(users, "reviewerUsers");
// Definimos tu ID estática para usuarios anónimos
const ANON_UUID = "bb50c6a4-d284-4cdd-8263-cf6b4a74de25";

// 🔍 1. CONSULTA GENERAL (Con doble JOIN para Ratings, Reviews y Autores de la Reseña)
export const getJobs = async (zip?: string) => {
  try {
    let query = db
      .select()
      .from(jobs)
      // JOIN 1: Creador del empleo
      .leftJoin(users, eq(jobs.userId, users.id)) 
      // JOIN 2: Calificación / Estrellas
      .leftJoin(ratingTable, eq(ratingTable.referenceId, jobs.id)) 
      // JOIN 3: Comentario / Texto de la reseña
      .leftJoin(reviewsTable, eq(reviewsTable.relationshipId, ratingTable.id)) 
      // 🚀 JOIN 4: Usuario autor de la reseña
      .leftJoin(reviewerUsers, eq(ratingTable.userId, reviewerUsers.id))
      .$dynamic(); 
    ...
    for (const row of rows) {
      ...
      if (!itemsMap.has(itemId)) {
        ...
        itemsMap.set(itemId, {
          ...
          reviews: [], 
          rating: 0 // Inicializamos en 0 neto
        });
      }

      // Si hay un rating válido, lo agregamos buscando su autor y comentario
      if (row.rating && row.rating.id) {
        const commentText = row.reviews?.comment || '';
        
        // 🚀 LÓGICA DEL NOMBRE: Validamos si es el UUID anónimo
        let rName = 'Anónimo';
        if (row.rating.userId !== ANON_UUID && row.reviewerUsers) {
        ...
        itemsMap.get(itemId).reviews.push({
           ...
           comment: commentText, 
           userName: rName, // 🚀 Añadimos el nombre del autor retornado al front
    ...
    const finalList = await Promise.all(Array.from(itemsMap.values()).map(async (item) => {
        ...
        if (fileName && fileName.trim() !== '' && !fileName.startsWith('http')) {
            ...
            if (!error && data?.signedUrl) {
                ...
                console.warn(`⚠️ Error firmando imagen de empleo ${item.id}:`, error.message);
            }
        }
...
// 🔍 2. CONSULTA INDIVIDUAL POR ID (Actualizado con autor de reseñas)
export const getJobById = async (id: string) => {
  try {
    const rows = await db
      ...
      .leftJoin(ratingTable, eq(ratingTable.referenceId, jobs.id))
      .leftJoin(reviewsTable, eq(reviewsTable.relationshipId, ratingTable.id))
      // 🚀 JOIN: Usuario autor de la reseña
      .leftJoin(reviewerUsers, eq(ratingTable.userId, reviewerUsers.id))
      .where(eq(jobs.id, id));
    ...
    for (const row of rows) {
      if (row.rating && row.rating.id) {
        ...
        jobFinal.reviews.push({
          ...
          stars: Number(row.rating.rating) || 0,
          comment: commentText,
          userName: rName, // 🚀 Añadimos el nombre del autor
          displayTime: new Date(row.rating.createdAt || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        });
      }
    }
    ...
    if (publicUrl && publicUrl.trim() !== '' && !publicUrl.startsWith('http')) {
        ...
        const { data, error } = await supabase.storage
            .from(NOMBRE_BUCKET).createSignedUrl(`jobs/${cleanName}`, 3600);
...
export const createJob = async (data: any) => {
  try {
    ...
    const payload: any = {
      ...
      rating: 0, // Se crea en 0 neto
    };
    
    console.log("📦 Payload preparado para inserción:", JSON.stringify(payload, null, 2));
...
export const createJobReview = async (data: any) => {
    try {
      // 🚀 ASIGNACIÓN ESTRICTA DEL ANÓNIMO: Si no hay userId, forzamos tu UUID Anónimo
      let validUserId = ANON_UUID; 
      if (data.userId && typeof data.userId === 'string' && data.userId.length > 20) {
          validUserId = data.userId;
      }
      ...
      const ratingPayload: any = {
        ...
        userId: validUserId, // Ahora guarda la cuenta real o la anónima oficial
      };
      ...
      return {
        ...
        userName: validUserId === ANON_UUID ? 'Anónimo' : (data.userName || 'Usuario') // Retornamos el estado exacto al front
      };

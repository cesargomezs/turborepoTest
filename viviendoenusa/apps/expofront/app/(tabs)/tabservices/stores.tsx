...
import {
  ...
  StyleSheet, useWindowDimensions, Keyboard, TouchableWithoutFeedback,
...
// Componente Universal que evita el error en Web
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (geo.length > 0) {
        ...
        setShowMarkers(true);
        if (Platform.OS !== 'web') mapRef.current?.animateToRegion(newCoords, 1000);
            ...
            <View style={styles.cardContent}>
              ...
              <View style={styles.headerRow}>
                ...
                <View style={styles.headerIcons}>
                  ...
                  <MaterialCommunityIcons name="store-plus-outline" size={40} color={isDark ? '#fff' : '#000'} style={{opacity: 0.2}} />
                </View>
              </View>
              ...
              <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 20 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                <View style={styles.formContainer}>
                  ...
                  {/* RESTAURADO: Chips de especialidad */}
                ...
                <View style={styles.mapContainer}>
                  <MapComponent 
                    ...
                    dataSource={DATA_SOURCE} mapKey={mapKey} onZoom={handleZoom}
                    onMarkerPress={(l: any) => { setResults([l]); setIsFilteredByMap(true); }}
                  />
                </View>
                ...
                <View style={styles.resultsWrapper}>
                  {results.length > 0 && (
                    <ThemedText style={{ fontSize: 13, marginBottom: 4, opacity: 0.7, fontWeight: '600' }}>
                      {results.length} {results.length === 1 ? (t.lawyerstab?.resultone || "Resultado") : (t.lawyerstab?.resultdomore || "Resultados") }
                    </ThemedText>
                  )}
                  ...
                  {/* RESTAURADO: Estilo original de tarjetas con 2 botones de acción */}
                  {results.map((lawyer) => {
                      ...
                      <View key={lawyer.id} style={[styles.lawyerCard, { backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : '#fff', marginBottom: 10 }]}>
                        ...
                        <View style={styles.actionGroup}>
                          <TouchableOpacity onPress={() => {
                            const url = Platform.OS === 'ios' ? `maps:0,0?q=${lawyer.name}@${lawyer.lat},${lawyer.lng}` : `geo:0,0?q=${lawyer.lat},${lawyer.lng}(${lawyer.name})`;
                            Linking.openURL(url);
